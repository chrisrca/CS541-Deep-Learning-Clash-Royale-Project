...
class CRGameState:
    ...
    @staticmethod
    def is_card_selected(card_image: np.ndarray, confidence: float = .9):
    ...
    @staticmethod
    def cards_in_hand(screenshot: np.ndarray, card_images: list[CRElement.Card], current_elixir: float, confidence_color: float = 0.6, confidence_gray = 0.55):    
        ...
        for card_h in cards_h:
            ...
            if(is_gray and current_elixir is not None):
                ...
                labelled_image_collection = {card.name: CRElement.not_ready_overlay(card.GRAY, current_elixir, card.cost) for card in card_images}
            else:
                labelled_image_collection = {card.name: card.BGR for card in card_images}
